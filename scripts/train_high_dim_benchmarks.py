...
def parse_args():
    ...
    predefined_hyperparams = load_hyperparameters.high_dimensional_data(
        args.dataset)
...
if __name__ == "__main__":
    ...
    X_train, X_test = train_test_split(X, train_size=num_train,
                                       random_state=2)
    ...
    for epoch in tqdm(np.arange(NUM_EPOCHS)):
        for X_batch, Y_batch in data_utils.DataLoader((X_train, Y_train),
            ...
            # Calculate training loss at the end of epoch
    ...
    # plot true marginals
    figure_name = save_file_name + '_true_marginals.png'
    bins = np.linspace(-4, 4, 50)
    fig = visualize.plot_multidim_marginals(Y_test, vmin=0, vmax=0.15,
                                            title=f'{dataset}: True Marginals')
    fig.savefig(reports_path + figure_name, bbox_inches='tight', dpi=300)

    # plot predictions
    figure_name = save_file_name + '_predicted_marginals.png'
    bins = np.linspace(-4, 4, 50)
    fig = visualize.plot_multidim_marginals(test_pred, vmin=0, vmax=0.15,
    ...
    num_iters_per_epoch = int((num_train) / BATCH_SIZE)
