...
def two_dimensional_data(data, rng=None, batch_size=200):
    """
    Generate two-dimensional benchmark datasets from FFJORD paper.
    Borrowed from https://github.com/rtqichen/ffjord/blob/master/lib/toy_data.py
    """
    if rng is None:
    ...
    if data == "swissroll":
        data = datasets.make_swiss_roll(n_samples=batch_size,
                                                noise=1.0, random_state=rng)[0]
        data = data.astype("float32")[:, [0, 2]]
        data /= 5
        return data

    elif data == "circles":
        data = datasets.make_circles(n_samples=batch_size, factor=.5,
                                             noise=0.08, random_state=rng)[0]
        ...
        data = datasets.make_moons(n_samples=batch_size, noise=0.1,
                                           random_state=rng)[0]
        data = data.astype("float32")
        ...
        x2_ = rng.rand(batch_size) - np.random.randint(0, 2,
                                                            batch_size) * 2
